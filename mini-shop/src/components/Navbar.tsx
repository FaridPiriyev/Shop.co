      ...
      <nav className="w-full bg-white border-b border-gray-100 sticky top-0 z-40">
        <div className="max-w-[1280px] mx-auto px-4 py-5 flex items-center justify-between gap-4 md:gap-10">
          <div className="flex items-center gap-4">
            ...
            <Link
              to="/"
              className="text-[28px] md:text-3xl tracking-tighter uppercase font-extrabold font-['Integral_CF',sans-serif] leading-none text-[#000000]"
            >
              SHOP.CO
            </Link>
          </div>

          <ul className="hidden md:flex items-center gap-6 text-base font-['Satoshi'] text-black">
          ...
          <div className="hidden lg:flex flex-1 items-center bg-[#F0F0F0] rounded-full px-4 py-3 gap-3 relative">
            ...
            {showResults && searchResults.length > 0 && (
              <div className="absolute top-full left-0 w-full bg-white border border-gray-100 mt-2 rounded-xl shadow-lg overflow-hidden z-50">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            >
              ...
              {cartCount > 0 && (
                <span className="absolute -top-2 -right-2 bg-black text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold border border-white animate-in zoom-in">
                  {cartCount}
                </span>
              )}
            </Link>
        ...
        {isMobileSearchOpen && (
          <div className="lg:hidden px-4 pb-4 animate-in fade-in slide-in-from-top-2 duration-300">
            <div className="flex items-center bg-[#F0F0F0] rounded-full px-4 py-3 gap-3 relative">
              <Search className="text-gray-400 w-5 h-5" />
              <input
                type="text"
                autoFocus
        ...
        >
          <div
            className={`fixed left-0 top-0 h-full w-[280px] bg-white p-6 transition-transform duration-500 ease-out ${
              isMenuOpen ? "translate-x-0" : "-translate-x-full"
            }`}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex justify-between items-center mb-10">
              <h2 className="text-2xl font-black text-black">SHOP.CO</h2>
              <button onClick={() => setIsMenuOpen(false)}>
                <X className="w-7 h-7 text-black" />
              </button>
            </div>
            <ul className="flex flex-col gap-6 text-xl font-bold text-black">
              <li className="flex justify-between items-center border-b pb-2">
                Shop <ChevronDown size={20} />
              </li>
              <li className="border-b pb-2">On Sale</li>
              <li className="border-b pb-2">New Arrivals</li>
              <li className="border-b pb-2">Brands</li>
              <Link
                to="/wishlist"
                onClick={() => setIsMenuOpen(false)}
